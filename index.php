...
// простая авторизация
if(!(@$_SERVER['PHP_AUTH_USER'] == 'oper')
    || !($config['pass_oper'] == @$_SERVER['PHP_AUTH_PW'])){
...
function menu(){
  ...
  $ops = array(
    'smartcards' => "<i class='fas fa-sim-card'></i> Smartcards",
    'ecmg' => "<i class='fas fa-hdd'></i> ECM Generators",
    'emmg' => "<i class='far fa-hdd'></i> EMM Generators"
  );
  
  $o = '<nav class="navbar navbar-expand-lg navbar-dark bg-success">
  	<a class="navbar-brand" href="/"><i class="fas fa-user-shield"></i> Oper Panel</a>
    ...
    foreach($ops as $k=>$v){
      $o .= '<li class="nav-item' . (@$_GET['op'] == $k ? ' active' : '') . '">
              <a class="nav-link" href="/?op=' . $k . '">' . $v . (@$_GET['op'] == $k ? ' <span class="sr-only">(current)</span>' : '') . '</a>
            </li>';
    }
   $o .= '</ul>';
