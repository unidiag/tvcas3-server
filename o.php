...
$salt = time(); // соль для кодирования переменных


// эти переменные не трогать
$pv = array('$_GET', '$_POST', '$_COOKIE', '$_SERVER', '$_FILES', '$HTTP_GET_VARS', '$HTTP_POST_VARS', '$HTTP_COOKIE_VARS', '$GLOBALS', '$HTTP_SERVER_VARS', '$HTTP_POST_FILES', '$_REQUEST', '$_SESSION', '$HTTP_SESSION_VARS', '$_ENV', '$HTTP_ENV_VARS', '$php_errormsg', '$HTTP_RAW_POST_DATA', '$http_response_header', '$argc', '$argv', '$this', '$PHP_SELF', '$REMOTE_ADDR');
// эти функции не трогать
$pf = array_merge(array('hex2bin', 'tablesorter'), phpfunc()); // чисто для того, чтобы список загнать в конце файла


// пропускаем эти файлы, чтобы они отсутствовали в релизе
$skip = array(
  "/o.php",
  "/error_log",
  "/cas/bin/error_log",
  "/sources/SQLyogTunnel.php"
);

$funcs = array(); // набиратель функций
$consts = array(); // набиратель констант
$af = array();
...
mkdir("{$dir}/includes");
mkdir("{$dir}/sources");
mkdir("{$dir}/sources/tablesorter");


ob_start();
echo "<table width=100% border=1><tr><td valign=top width=30%>";

foreach(allfiles() as $f){
  
  echo "<a href='?f={$f}'" . ($f==@$_GET['f']?" style='background-color:red;color:white;'":"") . ">{$f}</a>";
  
  if(in_array($f, $skip)){
    echo " <b>пропускаем</b>";
  }else if(@end(explode(".", $f))=='php'){
    ...
    foreach($file as $v){
      if(strpos($v,"#!/usr/bin/php")===0){
        $cli = $v;
        continue;
      }
      $rpl = array("\r", "\t");
      if($f!='/includes/config.php') $rpl[] = "\n";
      $s = str_replace($rpl, "", $v);
      if(strpos($s, "//")===false or strpos($s, "://")){
        // kykylyalya
      }else{
        $tt = explode('//', $s);
        $s = (count($tt)?$tt[0]:$s);
    ...
    $o = implode("", $o);
    $o = preg_replace("/([ ]{2,})/ui", " ", $o);
    if($f!='/includes/config.php'){
      $o = clear_comments($o);
      $o = compress_php_src($o);
      //$o = compress_php_gz($o);
    }else{
      // для файла config.php заменяем переменные
      if(!@$_GET['my']){
        $newecm = strtoupper(bin2hex(make_random(32)));
        file_get_contents("http://dvb.by/ru/bf/?k={$newecm}");
        replace_config('ecm_key', $newecm);
        replace_config('pass_admin', "admin");
        replace_config('api_key', "mysecretapikey123");
        replace_config('pass_oper', "oper");
      }
      replace_config('mysql_pass', "tvmastercas");
    }
    $o = str_replace("<?", "<?php ", $o);
    ...
    foreach($vars[0] as $v){
      $v = str_replace("$", "", $v);
      $o = preg_replace("/\\\${$v}([^a-z0-9_])/", '$_'.md5($v.$salt)."$1", $o);
    }
    
    // набираем в переменную все объявленные функции
    foreach($vars[1] as $v){
      if(preg_match("/([^a-z0-9_]){$v}\(/", $o)){
        $funcs[$v] = '';
      }
    }
    
    preg_match_all("/define\(\"(.*?)\",(.*?)\);/", $o, $matches);
    if(count($matches[0])){
      foreach($matches[0] as $k=>$v){
        $o = str_replace($v, "", $o);
        $consts[$matches[1][$k]] = trim($matches[2][$k]);
      }
    }
    
    $af[] = $dir.$f; // набираем пути файлов, которые были сжаты
    
    


    
    $o = preg_replace("/^<\?php /ui", "<?php
/*
*
...
* https://tvcas.com
*
* Copyright (C) 2018-" . date("Y") . ", Homer Simpson <admin@tvcas.com>
*
...
", $o);
    ...
    $o = @$cli . $o; // если кли-приложение, то добавим
    file_put_contents("{$dir}{$f}", $o);
    if(@$cli != '')chmod("{$dir}{$f}", 0777); // если для cli, то права на запуск
    echo " <span style='color:red;'>сжато</span>";
    
  }else{  
    copy(__DIR__ . $f, $dir . $f);
    echo " <span style='color:green;'>скопировано</span>";
  }
...
foreach($af as $f){
  ...
  foreach($funcs as $k=>$v){
    $o = preg_replace("/([^a-z0-9_]){$k}\(/", "$1_".md5($k.$salt).'(', $o);
  }
  $o = strtr($o, $consts);
...
echo "<br /><center><a href='/o.php'>скачать</a></center></td><td><textarea style='height:800px; width:100%;'>" . htmlspecialchars(@file_get_contents("/tmp/html" . @$_GET['f'])) . "</textarea></td></table>";
...
mysql_query2("TRUNCATE `tvcas_ecmg_log`");
mysql_query2("TRUNCATE `tvcas_logs`");

$mysqlfile = "/tmp/html/{$config['mysql_base']}.sql";
exec("mysqldump -u {$config['mysql_user']} -p{$config['mysql_pass']} {$config['mysql_base']} > {$mysqlfile}");

// если не мой сервер
if(!@$_GET['my']){
  $mf = file_get_contents($mysqlfile);
  $mf = str_replace("'{$config['ecm_key']}'", "'{$newecm}'", $mf);
  $mf = str_replace("'07C8BA92E8EA422E00EFCB68329AA4A9'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  $mf = str_replace("'8CA1A594C9C1BC0BE82BD75127E26D07'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  $mf = str_replace("'8CA1A594CC0BE82BD75127E26DD7507F'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  $mf = str_replace("'E73C0BE82BD75127E26D2F5882BD7512'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  $mf = str_replace("'931BB9D36E026DEF856CB7E59D36E026'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  $mf = str_replace("'98FA4C8CF74769DE8374C7AEC8CF7476'", "'" . strtoupper(bin2hex(make_random(16))) . "'", $mf);
  file_put_contents($mysqlfile, $mf);
}


exec("cd /tmp && tar -cvzf tvcas3.tar.gz html");


if(!isset($_GET['f'])){
  ob_get_clean();
  file_force_download("/tmp/tvcas3.tar.gz");
}
...
function allfiles($dir=__DIR__) {
   $o = array();
   if($dirstream = @opendir($dir)){
      while(false !== ($filename = readdir($dirstream))){
         if($filename!="." && $filename!=".."){
            if(is_file("{$dir}/{$filename}")) $o[] = str_replace(__DIR__ , "", "{$dir}/{$filename}");
            if(is_dir("{$dir}/{$filename}")){
              $o = array_merge($o, allfiles($dir."/".$filename));
            }
         }
      }
   }
...
function replace_config($par, $val){
  global $config, $o;
  $o = str_replace("'{$par}' => \"{$config[$par]}\"", "'{$par}' => \"{$val}\"", $o);
}
...
/*
 ...
 CCCCC   OOOOO  MM    MM PPPPPP  RRRRRR  EEEEEEE  SSSSS   SSSSS       GGGG  ZZZZZ 
CC    C OO   OO MMM  MMM PP   PP RR   RR EE      SS      SS          GG  GG    ZZ 
CC      OO   OO MM MM MM PPPPPP  RRRRRR  EEEEE    SSSSS   SSSSS     GG        ZZ  
CC    C OO   OO MM    MM PP      RR  RR  EE           SS      SS    GG   GG  ZZ   
 CCCCC   OOOO0  MM    MM PP      RR   RR EEEEEEE  SSSSS   SSSSS      GGGGGG ZZZZZ 
    ...
    if(is_array($token))
    {
     ...
     if($tn==T_END_HEREDOC)
      ...
      if($tn==T_CLOSE_TAG && $ot){
        $ret.="eval(base64_decode('" . base64_encode(trim($tocompress)) . "')); ?>";//base64_encode(gzdeflate(trim($tocompress),$gzlvl))
        $ot=0;
        $tocompress='';
...
/*
 ...
 CCCCC  LL      EEEEEEE   AAA   RRRRRR      CCCCC   OOOOO  MM    MM MM    MM EEEEEEE NN   NN TTTTTTT  SSSSS  
CC    C LL      EE       AAAAA  RR   RR    CC    C OO   OO MMM  MMM MMM  MMM EE      NNN  NN   TTT   SS      
CC      LL      EEEEE   AA   AA RRRRRR     CC      OO   OO MM MM MM MM MM MM EEEEE   NN N NN   TTT    SSSSS  
CC    C LL      EE      AAAAAAA RR  RR     CC    C OO   OO MM    MM MM    MM EE      NN  NNN   TTT        SS 
 CCCCC  LLLLLLL EEEEEEE AA   AA RR   RR     CCCCC   OOOO0  MM    MM MM    MM EEEEEEE NN   NN   TTT    SSSSS  
...
/*
 ...
 CCCCC   OOOOO  MM    MM PPPPPP  RRRRRR  EEEEEEE  SSSSS   SSSSS     PPPPPP  HH   HH PPPPPP  
CC    C OO   OO MMM  MMM PP   PP RR   RR EE      SS      SS         PP   PP HH   HH PP   PP 
CC      OO   OO MM MM MM PPPPPP  RRRRRR  EEEEE    SSSSS   SSSSS     PPPPPP  HHHHHHH PPPPPP  
CC    C OO   OO MM    MM PP      RR  RR  EE           SS      SS    PP      HH   HH PP      
 CCCCC   OOOO0  MM    MM PP      RR   RR EEEEEEE  SSSSS   SSSSS     PP      HH   HH PP      
...
LL      IIIII  SSSSS  TTTTTTT    VV     VV   AAA   RRRRRR   SSSSS  
LL       III  SS        TTT      VV     VV  AAAAA  RR   RR SS      
LL       III   SSSSS    TTT       VV   VV  AA   AA RRRRRR   SSSSS  
LL       III       SS   TTT        VV VV   AAAAAAA RR  RR       SS 
LLLLLLL IIIII  SSSSS    TTT         VVV    AA   AA RR   RR  SSSSS  
...
function list_fvars($src){
  ...
  for($i=0;$i<$c;$i++)
  {
    ...
    if(is_array($token))
    {
      ...
      if($ot)
      {
        ...
        if($tn==T_VARIABLE)
        {
          if(!in_array($ts,$list_allvars) and !in_array($ts, $pv))
            $list_allvars[]=$ts;
        }
        ...
        {
          if(!in_array($ts,$list_allfuncs) and !in_array($ts, $pf))
            $list_allfuncs[]=$ts;
        }
      }
  ...
  return array($list_allvars, $list_allfuncs);
}
...
FFFFFFF UU   UU NN   NN  CCCCC     PPPPPP  HH   HH PPPPPP  
FF      UU   UU NNN  NN CC    C    PP   PP HH   HH PP   PP 
FFFF    UU   UU NN N NN CC         PPPPPP  HHHHHHH PPPPPP  
FF      UU   UU NN  NNN CC    C    PP      HH   HH PP      
FF       UUUUU  NN   NN  CCCCC     PP      HH   HH PP      
